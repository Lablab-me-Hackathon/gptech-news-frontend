import MenuItems from "../services/header/MenuItems";
import Sign from "../services/header/Sign";
import { Menu } from "lucide-react";
import { useState } from "react";
import Logo from './../services/header/Logo';

function Header() {
  const [menuOpen, setMenuOpen] = useState(false);

  return (
    <div className="header flex-box px-4 justify-between w-full py-5 bg-gray-950 text-center">
      <Logo />
      <div className="hidden lg:block">
        <MenuItems />
      </div>
      <div className="hidden lg:block">
        <Sign />
      </div>
      <div
        className="text-white border p-2 rounded-lg hover:text-blue-950 hover:bg-white cursor-pointer block lg:hidden"
        onClick={() => {
          setMenuOpen((open) => !open);
        }}
      >
        <Menu />
        <div className="absolute top-20 left-0 bg-gray-950 flex-box flex-col pb-8 w-full">
          {menuOpen ? (
            <>
              {" "}
              <MenuItems /> <Sign />
            </>
          ) : (
            ""
          )}
        </div>
      </div>
    </div>
  );
}

export default Header;
